...
import { getCompanyAdmin, updateCompanyAdmin } from "../../assets/Apis/admin";
...
export default function EditCompany() {
  ...
  const fetchData=async()=>
    {
      const response=await getCompanyAdmin(TOKEN,companyID);
      if (response)
  ...
  const saveChanges = async () => {
      const validation = validateUpdateAssuranceAdmin(updates,"company");
      if (Object.keys(validation).length === 0) {
        const response = await updateCompanyAdmin(TOKEN,companyID, updates);
        if (response) {
          setShowDialog(true);
        }
      } else {
  ...
  return (
    <main className=" mx-auto my-8 px-4 md:px-6">
      <Card className="shadow-md hover:shadow-xl text-left rounded-2xl transistion-shadow duration-500 drop-shadow-md">
        ...
        <div className="flex items-center gap-4 mb-5">
          <Avatar className="h-14 w-14 ml-8">
            ...
            <AvatarFallback className="font-med">{formData?.name}</AvatarFallback>
          </Avatar>
          <div className="grid gap-0.5 text-sm ">
            <div className="font-medium text-center pb-1">{formData?.name?.[0]}{formData?.name?.[1]}</div>
          </div>
        </div>
      ...
      <AlertModal
        ...
        content="Company has been successfully updated."
        onClick={hideDialog}
        buttonTitle="Dismiss"
      />
    </main>
